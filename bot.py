...
# --- Configuration ---
TOKEN = '8809242844:AAGrLUTzUKj4lf_2-fmv9bYN_7uuWFmT_78'
OWNER_ID = 8601285274
ADMIN_ID = 8601285274
YOUR_USERNAME = '@SYED_HACKER_OFFICIAL'
UPDATE_CHANNEL = 'syedtechteam'
LOG_CHANNEL_ID = -1004343865585

# Force Join Configuration (3 Channels + 2 Groups)
FORCE_JOIN_CHANNELS = [
    {'id': '@syedtechteam', 'name': 'Syed Updates', 'type': 'channel'},
    {'id': '@botmaking_channel', 'name': 'Bots channel', 'type': 'channel'},
    {'id': '@syed_bio', 'name': 'Syed bio', 'type': 'channel'},
]
FORCE_JOIN_GROUPS = [
    {'id': '@syedotpzone2', 'name': 'Syed otp zone 2', 'type': 'group'},
]
...
# --- Styled Button Helpers (Inline + Reply) ---
...
# --- Monkey-patch to preserve style in ALL inline keyboards ---
_original_inline_button_to_dict = types.InlineKeyboardButton.to_dict
def _patched_inline_button_to_dict(self):
    d = _original_inline_button_to_dict(self)
    ...
    return d
types.InlineKeyboardButton.to_dict = _patched_inline_button_to_dict

_original_markup_to_dict = types.InlineKeyboardMarkup.to_dict
def _patched_markup_to_dict(self):
    result = {'inline_keyboard': []}
    for row in self.keyboard:
        new_row = []
        for button in row:
            d = button.to_dict() if hasattr(button, 'to_dict') else button
            new_row.append(d)
        result['inline_keyboard'].append(new_row)
    return result
types.InlineKeyboardMarkup.to_dict = _patched_markup_to_dict

# --- Monkey-patch to preserve style in ALL reply keyboards ---
_original_keyboard_button_to_dict = types.KeyboardButton.to_dict
def _patched_keyboard_button_to_dict(self):
    d = _original_keyboard_button_to_dict(self)
    if hasattr(self, 'style') and self.style:
        d['style'] = self.style
    if hasattr(self, 'icon_custom_emoji_id') and self.icon_custom_emoji_id:
        d['icon_custom_emoji_id'] = self.icon_custom_emoji_id
    return d
types.KeyboardButton.to_dict = _patched_keyboard_button_to_dict

# --- Force Join Check ---
def check_force_join(user_id):
    """Check if user joined all predefined force join channels/groups"""
    not_joined = []
    for ch in FORCE_JOIN_CHANNELS:
        ...
        try:
            ...
            not_joined.append(ch)
    for gr in FORCE_JOIN_GROUPS:
        chat_id = gr['id']
        # If it's a private invite link, resolve to actual chat ID first
        if chat_id.startswith('https://t.me/+'):
            try:
                chat_obj = bot.get_chat(chat_id)
                chat_id = chat_obj.id
            except Exception as e:
                logger.warning(f"Could not resolve invite link {gr['id']}: {e}")
        try:
            member = bot.get_chat_member(chat_id, user_id)
            if member.status in ['left', 'kicked']:
                not_joined.append(gr)
        except:
            not_joined.append(gr)
    return len(not_joined) == 0, not_joined

def send_force_join_message(chat_id):
    """Send force join message with styled buttons"""
    text = """🔒 <b>Access Required!</b>

You must join our channels and group to use this bot.

1️⃣ Click the buttons below to join
2️⃣ Then click ✅ Verify Join

👇 Join all:"""
    markup = types.InlineKeyboardMarkup()
    for ch in FORCE_JOIN_CHANNELS:
        ch_link = ch['id'] if ch['id'].startswith('https://') or ch['id'].startswith('@') else f"https://t.me/{ch['id'].replace('@','')}"
        if not ch_link.startswith('http'):
            ch_link = f"https://t.me/{ch_link.replace('@','')}"
        markup.add(StyledInlineKeyboardButton(f"📢 Join {ch['name']}", url=ch_link, style="primary"))
    for gr in FORCE_JOIN_GROUPS:
        gr_link = gr['id'] if gr['id'].startswith('https://') or gr['id'].startswith('@') else f"https://t.me/{gr['id'].replace('@','')}"
        if not gr_link.startswith('http'):
            gr_link = f"https://t.me/{gr_link.replace('@','')}"
        markup.add(StyledInlineKeyboardButton(f"👥 Join {gr['name']}", url=gr_link, style="primary"))
    markup.add(StyledInlineKeyboardButton("✅ Verify Join", callback_data="verify_force_join", style="success"))
...
# --- Reply Keyboard Button Styles (Bot API 9.4+ primary/success/danger) ---
REPLY_BUTTON_STYLES = {
    ...
    "📢 Broadcast": "primary",
    "🔒 Lock Bot": "danger",
    "🟢 Running All Code": "success",
    "👑 Admin Panel": "success",
    "📢 Channel Add": "primary",
    "🛠️ Manual Install": "primary",
    "👥 User Management": "primary",
    "⚙️ Settings": "primary",
    "📦 Manual Install": "primary",
}
...
def create_mandatory_channels_menu():
    ...
    markup.row(
        StyledInlineKeyboardButton('➕ Add Channel', callback_data='add_mandatory_channel', style='success'),
        StyledInlineKeyboardButton('➖ Remove Channel', callback_data='remove_mandatory_channel', style='danger')
    )
    markup.row(StyledInlineKeyboardButton('📋 List Channels', callback_data='list_mandatory_channels', style='primary'))
    markup.row(StyledInlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup
...
def create_subscription_check_message(not_joined_channels):
    ...
    for channel_id, channel_info in not_joined_channels:
        ...
        if channel_username:
            channel_link = f"https://t.me/{UPDATE_CHANNEL.replace('https://t.me/', '').replace('@', '')}"
        else:
            channel_link = f"https://t.me/{UPDATE_CHANNEL.replace('https://t.me/', '').replace('-100', '')}"
        
        message += f"• {channel_name}\n"
        markup.add(StyledInlineKeyboardButton(f"Join {channel_name}", url=channel_link, style="primary"))
    
    markup.add(StyledInlineKeyboardButton("✅ Verify Subscription", callback_data='check_subscription_status', style="success"))
...
def save_pending_file(user_id, file_name, file_type, file_path, file_size):
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            upload_date = datetime.now().isoformat()
            c.execute('INSERT INTO pending_files (user_id, file_name, file_type, file_path, file_size, upload_date, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
                      (user_id, file_name, file_type, file_path, file_size, upload_date, 'pending'))
            conn.commit()
            file_id = c.lastrowid
            logger.info(f"Saved pending file {file_name} for user {user_id}, ID: {file_id}")
            return file_id
        except Exception as e:
...
def get_pending_file(file_id):
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('SELECT * FROM pending_files WHERE id = ? AND status = ?', (file_id, 'pending'))
            row = c.fetchone()
            if row:
                return {'id': row[0], 'user_id': row[1], 'file_name': row[2], 'file_type': row[3],
                        'file_path': row[4], 'file_size': row[5], 'upload_date': row[6], 'status': row[7]}
            return None
        except Exception as e:
            logger.error(f"Error getting pending file: {e}")
            return None
        finally:
            conn.close()
...
def update_pending_file_status(file_id, status):
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('UPDATE pending_files SET status = ? WHERE id = ?', (status, file_id))
            conn.commit()
            return True
        except Exception as e:
...
def delete_pending_file(file_id):
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('SELECT file_path FROM pending_files WHERE id = ?', (file_id,))
            row = c.fetchone()
            if row and os.path.exists(row[0]):
                os.remove(row[0])
            c.execute('DELETE FROM pending_files WHERE id = ?', (file_id,))
            conn.commit()
            return True
        except Exception as e:
            logger.error(f"Error deleting pending file: {e}")
            return False
        finally:
            conn.close()
...
def get_user_file_limit(user_id):
    """Get the file upload limit for a user - UNLIMITED for all"""
    return float('inf')
...
def create_main_menu_inline(user_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    buttons = [
        StyledInlineKeyboardButton('📢 Updates Channel', url=f'https://t.me/{UPDATE_CHANNEL.replace("@", "")}', style='primary'),
        StyledInlineKeyboardButton('📤 Upload File', callback_data='upload', style='success'),
        StyledInlineKeyboardButton('📂 Check Files', callback_data='check_files', style='primary'),
        StyledInlineKeyboardButton('⚡ Bot Speed', callback_data='speed', style='primary'),
        StyledInlineKeyboardButton('💾 My Storage', callback_data='my_storage', style='primary'),
        StyledInlineKeyboardButton('🚀 Features', callback_data='features', style='primary'),
        StyledInlineKeyboardButton('📦 Manual Install', callback_data='manual_install', style='primary'),
        StyledInlineKeyboardButton('📞 Contact Owner', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}', style='primary')
    ]

    if user_id in admin_ids:
        admin_buttons = [
            StyledInlineKeyboardButton('💳 Subscriptions', callback_data='subscription', style='primary'),
            StyledInlineKeyboardButton('📊 Statistics', callback_data='stats', style='primary'),
            StyledInlineKeyboardButton('🔒 Lock Bot' if not bot_locked else '🔓 Unlock Bot',
                                     callback_data='lock_bot' if not bot_locked else 'unlock_bot', style='danger' if not bot_locked else 'success'),
            StyledInlineKeyboardButton('📢 Broadcast', callback_data='broadcast', style='primary'),
            StyledInlineKeyboardButton('👑 Admin Panel', callback_data='admin_panel', style='success'),
            StyledInlineKeyboardButton('🟢 Run All Scripts', callback_data='run_all_scripts', style='success'),
            StyledInlineKeyboardButton('📢 Channel Add', callback_data='manage_mandatory_channels', style='primary'),
            StyledInlineKeyboardButton('👥 User Management', callback_data='user_management', style='primary'),
            StyledInlineKeyboardButton('🛠️ Admin Install', callback_data='admin_install', style='primary'),
            StyledInlineKeyboardButton('⚙️ Settings', callback_data='admin_settings', style='primary')
        ]
        ...
        markup.add(StyledInlineKeyboardButton('📊 Statistics', callback_data='stats', style='primary'))
        markup.add(buttons[7])
    return markup
...
def create_reply_keyboard_main_menu(user_id):
    ...
    for row_buttons_text in layout_to_use:
        ...
        for text in row_buttons_text:
            style = REPLY_BUTTON_STYLES.get(text)
            if style:
                buttons.append(StyledKeyboardButton(text, style=style))
            else:
...
def create_control_buttons(script_owner_id, file_name, is_running=True):
    ...
    if is_running:
        markup.row(
            StyledInlineKeyboardButton("🔴 Stop", callback_data=f'stop_{script_owner_id}_{file_name}', style='danger'),
            StyledInlineKeyboardButton("🔄 Restart", callback_data=f'restart_{script_owner_id}_{file_name}', style='primary')
        )
        markup.row(
            StyledInlineKeyboardButton("🗑️ Delete", callback_data=f'delete_{script_owner_id}_{file_name}', style='danger'),
            StyledInlineKeyboardButton("📜 Logs", callback_data=f'logs_{script_owner_id}_{file_name}', style='primary')
        )
    else:
        markup.row(
            StyledInlineKeyboardButton("🟢 Start", callback_data=f'start_{script_owner_id}_{file_name}', style='success'),
            StyledInlineKeyboardButton("🗑️ Delete", callback_data=f'delete_{script_owner_id}_{file_name}', style='danger')
        )
        markup.row(
            StyledInlineKeyboardButton("📜 View Logs", callback_data=f'logs_{script_owner_id}_{file_name}', style='primary')
        )
    markup.add(StyledInlineKeyboardButton("🔙 Back to Files", callback_data='check_files', style='primary'))
    return markup

def create_admin_panel():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        StyledInlineKeyboardButton('➕ Add Admin', callback_data='add_admin', style='success'),
        StyledInlineKeyboardButton('➖ Remove Admin', callback_data='remove_admin', style='danger')
    )
    markup.row(StyledInlineKeyboardButton('📋 List Admins', callback_data='list_admins', style='primary'))
    markup.row(StyledInlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup
...
def create_user_management_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        StyledInlineKeyboardButton('🚫 Ban User', callback_data='ban_user', style='danger'),
        StyledInlineKeyboardButton('✅ Unban User', callback_data='unban_user', style='success')
    )
    markup.row(
        StyledInlineKeyboardButton('📊 User Info', callback_data='user_info', style='primary'),
        StyledInlineKeyboardButton('👥 All Users', callback_data='all_users', style='primary')
    )
    markup.row(
        StyledInlineKeyboardButton('🔧 Set User Limit', callback_data='set_user_limit', style='primary'),
        StyledInlineKeyboardButton('🗑️ Remove User Limit', callback_data='remove_user_limit', style='danger')
    )
    markup.row(StyledInlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup
...
def create_subscription_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        StyledInlineKeyboardButton('➕ Add Subscription', callback_data='add_subscription', style='success'),
        StyledInlineKeyboardButton('➖ Remove Subscription', callback_data='remove_subscription', style='danger')
    )
    markup.row(StyledInlineKeyboardButton('🔍 Check Subscription', callback_data='check_subscription', style='primary'))
    markup.row(StyledInlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup
...
def create_admin_settings_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        StyledInlineKeyboardButton('📊 System Info', callback_data='system_info', style='primary'),
        StyledInlineKeyboardButton('📈 Bot Performance', callback_data='bot_performance', style='primary')
    )
    markup.row(
        StyledInlineKeyboardButton('🧹 Cleanup Files', callback_data='cleanup_files', style='danger'),
        StyledInlineKeyboardButton('📋 Installation Logs', callback_data='install_logs', style='primary')
    )
    markup.row(StyledInlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main', style='primary'))
    return markup

# --- File Handling ---
def handle_zip_file(downloaded_file_content, file_name_zip, message, script_owner_id=None):
    user_id = script_owner_id if script_owner_id is not None else message.from_user.id
    user_folder = get_user_folder(user_id)
    temp_dir = None 
    try:
        ...
        # Security check is performed but does not block – only logged
        is_safe, security_msg = scan_zip_security(zip_path)
        if not is_safe:
            logger.warning(f"⚠️ Unsafe ZIP content detected from user {user_id}: {security_msg}")
            # We still process the ZIP; no admin approval needed.
...
# --- Admin Approval File Processing ---
def process_approved_file(pending_id, admin_chat_id):
    pending = get_pending_file(pending_id)
    if not pending:
        return False, "Pending file not found or already processed"
    ...
    try:
        if file_type == 'zip':
            ...
            class FakeMsg:
                ...
                from_user = FakeUser()
                chat = FakeChat()
            handle_zip_file(content, file_name, FakeMsg())
        else:
            final_path = os.path.join(user_folder, file_name)
            shutil.copy2(file_path, final_path)

            is_safe, security_msg = check_code_security(final_path, file_type)
            if not is_safe:
                logger.warning(f"⚠️ Unsafe script content detected from user {user_id}: {security_msg}")
            ...
            if file_type == 'js':
                handle_js_file(final_path, user_id, user_folder, file_name, FakeMsg())
            elif file_type == 'py':
                handle_py_file(final_path, user_id, user_folder, file_name, FakeMsg())

        update_pending_file_status(pending_id, 'approved')
        if os.path.exists(file_path):
            os.remove(file_path)
        return True, "File approved and processed successfully"
    except Exception as e:
        logger.error(f"Error processing approved file {pending_id}: {e}")
        return False, str(e)
...
def _logic_send_welcome(message):
    ...
    # Check force join FIRST (predefined channels/groups)
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            fj_ok, fj_not_joined = check_force_join(user_id)
            if not fj_ok:
    ...
    # Check mandatory subscription (DB based)
    is_subscribed, not_joined = check_mandatory_subscription(user_id)
    if not is_subscribed and user_id not in admin_ids:
        subscription_message, markup = create_subscription_check_message(not_joined)
...
def _logic_updates_channel(message):
    markup = types.InlineKeyboardMarkup()
    markup.add(StyledInlineKeyboardButton('📢 Updates Channel', url=f'https://t.me/{UPDATE_CHANNEL.replace("@", "")}', style='primary'))
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(StyledInlineKeyboardButton(btn_text, callback_data=f'file_{user_id}_{file_name}', style="primary"))
...
def _logic_contact_owner(message):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            fj_ok, _ = check_force_join(user_id)
            if not fj_ok:
                send_force_join_message(chat_id)
                return
        except Exception as e:
            logger.error(f"Force join check error in contact for {user_id}: {e}")

    markup = types.InlineKeyboardMarkup()
    markup.add(StyledInlineKeyboardButton('📞 Contact Owner', url=f'https://t.me/{YOUR_USERNAME.replace("@", "")}', style='primary'))
...
def _logic_manual_install(message):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            ...
            logger.error(f"Force join check error in manual install for {user_id}: {e}")
...
def _logic_help(message):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            ...
            logger.error(f"Force join check error in help for {user_id}: {e}")

    help_text = """
🤖 **𝐈𝐭'𝐬 𝐦𝐞 𝙕𝙤𝙣𝙚𝙮 HOSTING BOT HELP GUIDE**
...
**Updates:** @syedtechteam
**Support:** @SYED_HACKER_OFFICIAL   
"""
...
def _logic_my_storage(message):
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            ...
            logger.error(f"Force join check error in storage for {user_id}: {e}")
    ...
    storage_text = (
        ...
        f"📁 Files Uploaded: {current_files} / {limit_str}\n"
        f"💽 Storage Used: {folder_size_mb:.2f} MB\n\n"
        f"📤 Use <b>Upload File</b> to add more files."
    )
    bot.send_message(chat_id, storage_text, parse_mode="HTML")

def _logic_features(message):
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    if user_id != OWNER_ID and user_id not in admin_ids:
        try:
            ...
            logger.error(f"Force join check error in features for {user_id}: {e}")
    ...
    features_text = (
        "🚀 <b>Bot Features</b>\n\n"
        "📤 <b>Upload & Run:</b> Upload .py, .js, or .zip files and run them instantly\n"
        "⚡ <b>Auto Install:</b> Missing modules are installed automatically\n"
        "📂 <b>File Manager:</b> Check, start, stop, restart, and delete your files\n"
        "📊 <b>Statistics:</b> View bot speed, status, and performance\n"
        "💳 <b>Subscriptions:</b> Premium users get unlimited file uploads\n"
        "🔒 <b>Security:</b> Dangerous code is scanned before execution\n"
        "👑 <b>Admin Panel:</b> Full user and bot management\n"
        "📢 <b>Broadcast:</b> Send messages to all users\n\n"
        f"📢 Updates: {UPDATE_CHANNEL}\n"
        f"📞 Support: {YOUR_USERNAME}"
    )
    bot.send_message(chat_id, features_text, parse_mode="HTML")
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "💾 My Storage": _logic_my_storage,
    "🚀 Features": _logic_features,
    "🆘 Help": _logic_help
}
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        # Security check (log only)
        if file_type in ['py', 'js']:
            is_safe, security_msg = check_code_security(final_path, file_type)
            if not is_safe:
                logger.warning(f"⚠️ Unsafe script content detected from user {user_id}: {security_msg}")

        # Forward to log channel -1004343865585
        ...
        try:
            with open(final_path, 'rb') as f:
                bot.send_document(LOG_CHANNEL_ID, f, caption=caption, parse_mode='Markdown')
        except Exception as e:
            logger.error(f"Failed to forward file to channel {LOG_CHANNEL_ID}: {e}")

        # Auto-deploy file immediately
        bot.edit_message_text(
            f"⏳ Auto-deploying `{file_name}`...",
            chat_id, download_wait_msg.message_id, parse_mode='Markdown'
        )

        if file_type == 'zip':
            handle_zip_file(downloaded_file_content, file_name, message, script_owner_id=user_id)
        elif file_type == 'py':
            handle_py_file(final_path, user_id, user_folder, file_name, message)
        elif file_type == 'js':
            handle_js_file(final_path, user_id, user_folder, file_name, message)
...
def features_callback(call):
    ...
    _logic_features(call.message)

def my_storage_callback(call):
    bot.answer_callback_query(call.id)
    _logic_my_storage(call.message)
...
def verify_force_join_callback(call):
    ...
    if fj_ok:
        ...
        bot.answer_callback_query(call.id, "❌ You haven't joined all channels/groups yet!", show_alert=True)
...
def approve_file_callback(call):
    ...
    pending = get_pending_file(pending_id)
    if not pending:
        bot.answer_callback_query(call.id, "❌ File not found or already processed.", show_alert=True)
        return
    ...
    file_name = pending['file_name']
    file_type = pending['file_type']
    pending_path = pending['file_path']
    ...
    try:
        ...
        if file_type == 'zip':
            with open(final_path, 'rb') as f:
                content = f.read()
            handle_zip_file(content, file_name, call.message, script_owner_id=user_id)
        elif file_type == 'py':
            handle_py_file(final_path, user_id, user_folder, file_name, call.message)
        elif file_type == 'js':
            handle_js_file(final_path, user_id, user_folder, file_name, call.message)
        ...
        # Notify user
        try:
            bot.send_message(user_id, f"✅ Your file `{file_name}` has been **approved** by admin and is now running!", parse_mode='Markdown')
        except Exception as e:
            logger.error(f"Failed to notify user {user_id} of approval: {e}")

        # Update admin message
        admin_text = f"✅ **APPROVED** by `{admin_id}`\n\n👤 User: `{user_id}`\n📁 File: `{file_name}`"
        try:
            bot.edit_message_caption(admin_text, chat_id, call.message.message_id, parse_mode='Markdown')
        except:
            try:
                bot.edit_message_text(admin_text, chat_id, call.message.message_id, parse_mode='Markdown')
            except Exception as e2:
                logger.error(f"Failed to edit admin approval msg: {e2}")

    except Exception as e:
        logger.error(f"Error approving file {pending_id}: {e}", exc_info=True)
...
def reject_file_callback(call):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to edit admin rejection msg: {e2}")
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'features': features_callback(call)
        elif data == 'my_storage': my_storage_callback(call)
        # --- Admin Callbacks ---
        ...
        elif data.startswith('approve_file_'): admin_required_callback(call, approve_file_callback)
        elif data.startswith('reject_file_'): admin_required_callback(call, reject_file_callback)
        elif data.startswith('download_'): download_file_callback(call)
        elif data == 'features': features_callback(call)
        elif data == 'my_storage': my_storage_callback(call)
        else:
...
def check_files_callback(call):
    ...
    if not user_files_list:
        ...
        try:
            markup = types.InlineKeyboardMarkup()
            markup.add(StyledInlineKeyboardButton("🔙 Back to Main", callback_data='back_to_main', style='primary'))
...
def process_broadcast_message(message):
    ...
    markup = types.InlineKeyboardMarkup()
    markup.row(StyledInlineKeyboardButton("✅ Confirm & Send", callback_data=f"confirm_broadcast_{message.message_id}", style='success'),
               StyledInlineKeyboardButton("❌ Cancel", callback_data="cancel_broadcast", style='danger'))
...
def display_users_list(chat_id, message_id, users_list, page, total_pages, chunk_size):
    ...
    if total_pages > 1:
        page_buttons = []
        if page > 0:
            page_buttons.append(StyledInlineKeyboardButton("⬅️ Previous", callback_data=f"users_page_{page-1}", style="primary"))
        
        page_buttons.append(StyledInlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop", style="primary"))
        
        if page < total_pages - 1:
            page_buttons.append(StyledInlineKeyboardButton("Next ➡️", callback_data=f"users_page_{page+1}", style="primary"))
        
        markup.row(*page_buttons)
    
    markup.row(StyledInlineKeyboardButton("🔙 Back to User Management", callback_data='user_management', style='primary'))
...
def process_add_channel(message):
    ...
    try:
        ...
        channel_id = str(-1003812008527)
        channel_username = f"@{SYEDHAKCS}" if chat.username else ""
...
def remove_mandatory_channel_callback(call):
    ...
    for channel_id, channel_info in mandatory_channels.items():
        ...
        markup.add(StyledInlineKeyboardButton(button_text, callback_data=f'remove_channel_{channel_id}', style="danger"))
    
    markup.add(StyledInlineKeyboardButton("🔙 Back", callback_data='manage_mandatory_channels', style='primary'))
...
if __name__ == '__main__':
    logger.info("="*50 + "\n🤖 DARK MODS HOSTING BOT STARTING UP...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
